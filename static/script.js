...
function displayRsiResults(results) {
    ...
    tbody.innerHTML = results.map(stock => {
        ...
        return `<tr>
            ...
            <td><span class="rec-badge ${rec.color || 'gray'}">${rec.score_100 != null ? rec.score_100 + '/100' : (rec.status || rec.action || 'محايد')}</span></td>
...
function displayResult(data) {
    ...
    const status = rec.status || rec.action || 'حالة محايدة';
    const score100 = rec.score_100 != null ? rec.score_100 : 50;
    set('rec-action', status);
    set('rec-score', `درجة الحالة: ${score100}/100`);
    if (recCard) {
        recCard.className = 'card rec-card';
        if (score100 >= 75) recCard.classList.add('rec-strong-buy');
        else if (score100 >= 60) recCard.classList.add('rec-buy');
        else if (score100 <= 24) recCard.classList.add('rec-strong-sell');
        else if (score100 <= 39) recCard.classList.add('rec-sell');
...
function generateAIDeepAnalysis(data) {
    ...
    const summaryMap = {
        strong_bullish: `السهم في صعود قوي جداً. جميع المؤشرات تدعم الاتجاه الصعودي. أقرب مستوى فني للمراقبة عند ${formatPrice(target1, data.currency)} (+${upPct}%).`,
        bullish:        `السهم في اتجاه صاعد. معظم المؤشرات إيجابية. مستوى دعم فني قريب عند ${formatPrice(stopLoss, data.currency)} (-${downPct}%).`,
        neutral:        'السهم في تذبذب جانبي بدون إشارة فنية واضحة حالياً.',
        bearish:        'السهم تحت ضغط بيعي وفق المؤشرات الفنية الحالية.',
        strong_bearish: 'السهم في هبوط حاد وفق المؤشرات الفنية — الزخم سلبي بوضوح.',
    };
