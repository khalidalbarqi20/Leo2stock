...
class BacktestingEngine:
    ...
    def run_backtest(self, prices, strategy='sma_cross', initial_capital=10000,
        ...
        if strategy == 'sma_cross':
            trades = self._sma_cross_strategy(closes, indicators_series, stop_loss_pct, take_profit_pct)
        elif strategy == 'rsi_reversal':
            trades = self._rsi_reversal_strategy(closes, indicators_series, stop_loss_pct, take_profit_pct)
        elif strategy == 'macd_signal':
            trades = self._macd_strategy(closes, indicators_series, stop_loss_pct, take_profit_pct)
        elif strategy == 'bollinger_bounce':
            trades = self._bollinger_strategy(closes, highs, lows, indicators_series, stop_loss_pct, take_profit_pct)
        elif strategy == 'golden_cross':
            trades = self._golden_cross_strategy(closes, indicators_series, stop_loss_pct, take_profit_pct)
        elif strategy == 'combined':
            trades = self._combined_strategy(closes, highs, lows, indicators_series, stop_loss_pct, take_profit_pct)
        else:
    ...
    def _calculate_indicators_series(self, prices):
        ...
        return {
            ...
            'rsi': rsi, 'macd': macd_line, 'signal': signal_line,
            'histogram': histogram, 'bb_upper': bb_upper,
            'bb_lower': bb_lower, 'bb_middle': bb_middle, 'atr': atr,
        }
        ...
        for i in range(50, len(closes)):
            ...
            if prev_sma20 <= prev_sma50 and curr_sma20 > curr_sma50:
                if position == 'long':
                    continue
                if position == 'short':
                    trades.append({'type': 'close_short', 'entry': entry_price,
                                   'exit': closes[i], 'entry_day': entry_day, 'exit_day': i, 'reason': 'crossover'})
                position = 'long'
                entry_price = closes[i]
                entry_day = i
                trades.append({'type': 'buy', 'price': entry_price, 'day': i, 'reason': 'SMA20 crosses above SMA50'})
                ...
                if position == 'long':
                    ...
                    reason = 'stop_loss' if current_price <= entry_price * (1 - stop_loss_pct) else \
                             'take_profit' if current_price >= entry_price * (1 + take_profit_pct) else \
                             'SMA20 crosses below SMA50'
                    trades.append({'type': 'sell', 'entry': entry_price, 'exit': current_price,
                                   'entry_day': entry_day, 'exit_day': i, 'reason': reason})
                    position = None
            ...
            if position == 'long':
                ...
                if current_price <= entry_price * (1 - stop_loss_pct):
                    trades.append({'type': 'sell', 'entry': entry_price, 'exit': current_price,
                                   'entry_day': entry_day, 'exit_day': i, 'reason': 'stop_loss'})
                    position = None
                elif current_price >= entry_price * (1 + take_profit_pct):
                    trades.append({'type': 'sell', 'entry': entry_price, 'exit': current_price,
                                   'entry_day': entry_day, 'exit_day': i, 'reason': 'take_profit'})
                    position = None

        if position == 'long':
            trades.append({'type': 'sell', 'entry': entry_price, 'exit': closes[-1],
                           'entry_day': entry_day, 'exit_day': len(closes) - 1, 'reason': 'end_of_data'})
        return trades
    ...
    def _rsi_reversal_strategy(self, closes, inds, stop_loss_pct, take_profit_pct):
        ...
        for i in range(14, len(closes)):
            ...
            if prev_rsi > 30 and current_rsi <= 30 and position is None:
                position = 'long'
                entry_price = current_price
                entry_day = i
                trades.append({'type': 'buy', 'price': entry_price, 'day': i,
                               'reason': f'RSI crossed below 30 ({current_rsi:.1f})'})

            elif prev_rsi < 70 and current_rsi >= 70 and position == 'long':
                trades.append({'type': 'sell', 'entry': entry_price, 'exit': current_price,
    ...
    def _macd_strategy(self, closes, inds, stop_loss_pct, take_profit_pct):
        ...
        for i in range(35, len(closes)):
            ...
            if macd[i-1] <= signal[i-1] and macd[i] > signal[i] and histogram[i] > 0 and position is None:
                position = 'long'
                entry_price = current_price
                entry_day = i
                trades.append({'type': 'buy', 'price': entry_price, 'day': i, 'reason': 'MACD crossed above Signal'})

            elif macd[i-1] >= signal[i-1] and macd[i] < signal[i] and position == 'long':
                trades.append({'type': 'sell', 'entry': entry_price, 'exit': current_price,
                               'entry_day': entry_day, 'exit_day': i, 'reason': 'MACD crossed below Signal'})
                position = None
    ...
    def _bollinger_strategy(self, closes, highs, lows, inds, stop_loss_pct, take_profit_pct):
        ...
        for i in range(20, len(closes)):
            ...
            if prev_price <= bb_lower[i-1] and current_price > bb_lower[i] and current_price > prev_price and position is None:
                position = 'long'
                entry_price = current_price
                entry_day = i
                trades.append({'type': 'buy', 'price': entry_price, 'day': i, 'reason': 'Bollinger Lower Band bounce'})

            elif prev_price >= bb_upper[i-1] and current_price < bb_upper[i] and position == 'long':
                trades.append({'type': 'sell', 'entry': entry_price, 'exit': current_price,
                               'entry_day': entry_day, 'exit_day': i, 'reason': 'Bollinger Upper Band rejection'})
                position = None

            elif position == 'long' and prev_price < bb_middle[i-1] and current_price >= bb_middle[i]:
                trades.append({'type': 'sell', 'entry': entry_price, 'exit': current_price,
                               'entry_day': entry_day, 'exit_day': i, 'reason': 'Bollinger Middle Band'})
                position = None
    ...
    def _golden_cross_strategy(self, closes, inds, stop_loss_pct, take_profit_pct):
        ...
        for i in range(200, len(closes)):
            ...
            if sma50[i-1] <= sma200[i-1] and sma50[i] > sma200[i] and position is None:
                position = 'long'
                entry_price = current_price
                entry_day = i
                trades.append({'type': 'buy', 'price': entry_price, 'day': i, 'reason': 'Golden Cross'})

            elif sma50[i-1] >= sma200[i-1] and sma50[i] < sma200[i] and position == 'long':
                trades.append({'type': 'sell', 'entry': entry_price, 'exit': current_price,
                               'entry_day': entry_day, 'exit_day': i, 'reason': 'Death Cross'})
                position = None
    ...
    def _calculate_performance(self, trades, closes, initial_capital, commission):
        ...
        if total_trades == 0:
            return {'total_trades': 0, 'winning_trades': 0, 'losing_trades': 0,
                    'win_rate': 0, 'total_return_pct': 0, 'annualized_return': 0,
                    'max_drawdown_pct': 0, 'sharpe_ratio': 0, 'profit_factor': 0,
                    'avg_trade_return': 0, 'avg_win': 0, 'avg_loss': 0,
                    'best_trade': 0, 'worst_trade': 0, 'final_capital': initial_capital,
                    'trades': trades, 'equity_curve': [initial_capital]}
        ...
        annualized_return = ((capital / initial_capital) ** (1 / max(years, 0.1)) - 1) * 100
        ...
        return {
            ...
            'win_rate': round(win_rate, 2), 'total_return_pct': round(total_return_pct, 2),
            'annualized_return': round(annualized_return, 2),
            'max_drawdown_pct': round(max_drawdown * 100, 2),
            'sharpe_ratio': round(sharpe_ratio, 2), 'profit_factor': round(profit_factor, 2),
            'avg_trade_return': round(avg_trade_return, 2),
            'avg_win': round(avg_win, 2), 'avg_loss': round(avg_loss, 2),
            'best_trade': round(best_trade, 2), 'worst_trade': round(worst_trade, 2),
            'final_capital': round(capital, 2), 'initial_capital': initial_capital,
    ...
    def compare_strategies(self, prices, initial_capital=10000):
        ...
        for strategy_key, strategy_name in self.STRATEGIES.items():
            result = self.run_backtest(prices, strategy=strategy_key, initial_capital=initial_capital)
            if 'error' not in result:
                results[strategy_key] = {'name': strategy_name, **result}
        return results
