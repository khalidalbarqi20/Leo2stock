...
class NewsAnalyzer:
    ...
    def get_news(self, symbol, market='us', limit=10):
        ...
        if not FINNHUB_KEY:
            result = {'symbol': symbol, 'status': 'NO_NEWS_SOURCE_CONFIGURED', 'total_articles': 0,
                       'articles': [], 'overall_sentiment': None}
            self.cache[cache_key] = (datetime.now(), result)
            return result

        if not news_data or not isinstance(news_data, list) or len(news_data) == 0:
            result = {'symbol': symbol, 'status': 'NO_NEWS_AVAILABLE', 'total_articles': 0,
                       'articles': [], 'overall_sentiment': None}
            self.cache[cache_key] = (datetime.now(), result)
            return result
        ...
        for article in news_data[:limit]:
            ...
            try:
                if published_at:
                    age_hours = round((now_ts - datetime.fromtimestamp(int(published_at))).total_seconds() / 3600, 1)
            except Exception:
                age_hours = None
