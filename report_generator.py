...
class ReportGenerator:
    """
    استيراد reportlab/matplotlib مؤجَّل (Lazy Import) عمداً — عشان لو ما كانت هذي المكتبات
    مثبّتة (مثلاً matplotlib صعب يُبنى على بعض أجهزة Termux)، باقي التطبيق (Dashboard/
    Scanner/Strategies) يشتغل عادي، وبس ميزة تصدير PDF هي اللي تفشل برسالة واضحة.
    """
    def __init__(self):
    ...
    def create_chart(self, prices, symbol):
        try:
            ...
            raise RuntimeError(
                "matplotlib غير مثبّت — تصدير الرسم البياني للـPDF غير متاح حتى تثبّته "
    ...
    def generate_pdf(self, symbol, data, analysis):
        try:
            ...
            raise RuntimeError(
                "reportlab غير مثبّت — تصدير PDF غير متاح حتى تثبّته "
