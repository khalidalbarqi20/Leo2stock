...
class TechnicalAnalyzer:
    ...
    def calculate_fibonacci(self, prices):
        ...
        if not highs or not lows:
            return {}
        high = max(highs[-60:]) if len(highs) >= 60 else max(highs)
        low = min(lows[-60:]) if len(lows) >= 60 else min(lows)
        if high == low:
            return {}
        diff = high - low
        return {
            '0': round(high, 2), '23.6': round(high - diff * 0.236, 2),
            '38.2': round(high - diff * 0.382, 2), '50': round(high - diff * 0.5, 2),
            '61.8': round(high - diff * 0.618, 2), '78.6': round(high - diff * 0.786, 2),
            '100': round(low, 2), 'high': round(high, 2), 'low': round(low, 2)
        }

    def calculate_volume_profile(self, prices):
        closes = self._get_closes(prices)
        ...
        for i in range(num_bins):
            ...
            vol_sum = sum(volumes[j] for j in range(len(closes)) if bin_low <= closes[j] < bin_high)
            mid_price = (bin_low + bin_high) / 2
            volume_by_price[round(mid_price, 2)] = vol_sum
        ...
        for price, vol in sorted_prices:
            cumulative += vol
            if cumulative >= target_vol * 0.15 and val == poc_price:
                val = price
            if cumulative >= target_vol * 0.85 and vah == poc_price:
                vah = price
        return {
            'poc': poc_price, 'poc_volume': poc_volume,
    ...
    def _calculate_macd_series(self, closes):
        ...
        return {'macd': macd_padded, 'signal': signal_padded, 'histogram': hist_padded}
    ...
    def get_chart_data(self, prices):
        ...
        return {
            'prices_list': closes, 'opens_list': opens, 'highs_list': highs,
            'lows_list': lows, 'volumes_list': volumes,
            'sma20_list': sma20, 'sma50_list': sma50, 'sma200_list': sma200,
            'rsi_list': rsi, 'macd_list': macd_data['macd'],
            'signal_list': macd_data['signal'], 'histogram_list': macd_data['histogram'],
        }
    ...
    def calculate_targets(self, prices, indicators):
        ...
        if len(recent_lows) >= 4:
            supports = [round(recent_lows[i], 2) for i in range(4)]
        elif recent_lows:
        ...
        if len(recent_highs) >= 4:
            targets = [round(recent_highs[i], 2) for i in range(4)]
        elif recent_highs:
        ...
        targets = targets[:4]
        return {
            'target_1': targets[0], 'target_2': targets[1], 'target_3': targets[2], 'target_4': targets[3],
            'stop_loss': stop_loss,
            'support_1': supports[0], 'support_2': supports[1], 'support_3': supports[2], 'support_4': supports[3],
        }
    ...
    def _get_fundamental_reason(self, trend, indicators, current, support, resistance):
        ...
        if trend in ['strong_bullish', 'bullish']:
            ...
            return {'direction': 'up', 'title': 'أسباب الارتفاع', 'reasons': reasons, 'summary': 'السهم في اتجاه صعودي مدعوم بمؤشرات فنية إيجابية'}
            ...
            return {'direction': 'down', 'title': 'أسباب الهبوط', 'reasons': reasons, 'summary': 'السهم في اتجاه هبوطي مدعوم بمؤشرات فنية سلبية'}
        else:
            if abs(rsi - 50) < 10:
            ...
            return {'direction': 'neutral', 'title': 'الوضع الحالي', 'reasons': reasons, 'summary': 'السهم في منطقة محايدة بانتظار محفزات جديدة'}
