...
class TechnicalAnalyzer:
    ...
    def get_recommendation(self, analysis):
        """
        يحسب 'درجة حالة السهم' (0-100) ووصفاً نوعياً بدون توصية شراء/بيع مباشرة.
        القرار النهائي يبقى للمستخدم دائماً — هذا مؤشر حالة فنية فقط.
        """
        ...
        # تحويل النطاق التقريبي (-5..+5) إلى درجة حالة من 0 إلى 100
        score_100 = max(0, min(100, round((raw_score + 5) / 10 * 100)))

        if score_100 >= 75:
            status, color = 'إشارة فنية قوية — تستحق المراجعة', 'green'
        elif score_100 >= 60:
            status, color = 'إشارة إيجابية', 'lightgreen'
        elif score_100 >= 40:
            status, color = 'حالة محايدة', 'gray'
        elif score_100 >= 25:
            status, color = 'إشارة ضعف', 'orange'
        else:
            status, color = 'إشارة سلبية واضحة', 'red'

        return {
            'status': status,       # الوصف النوعي (بدون أمر شراء/بيع)
            'action': status,       # اسم متوافق مع الواجهة القديمة
            'score_100': score_100, # درجة الحالة من 100
            'score': raw_score,     # للتوافق مع الحسابات القديمة
            'color': color,
        }
